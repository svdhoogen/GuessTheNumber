...
function isNumberKey(e)
{
    ...
    if (key < 32 || (key > 47 && key < 58)) { return true; } else { return false; } // If the key is a functional key(backspace, tab etc.), or is a number(48-57), handle keypress, otherwise, don't handle keypress.
}

// Event which ensures compatability between different browsers for keyevent.
function getKeyEvent(e){
    var keyNum; if(window.event) { keyNum = e.keyCode } else if(e.which) { keyNum = e.which } return keyNum; // Checks to see which browser it is on and performs logic to return keyNum for that browser, for compat.
}
...
function maxNumVerify(minInputNum) {
    var inputNum = document.getElementById("inputNum"); var num = inputNum.value; // Define element and get string to variable num..

    if (num == "") { inputNum.value = minInputNum; } // If field is empty, fill it with minimum expected input.
    while (num.length > 3) { num = maxNum.substring(0, num.length - 1); inputNum.value = num; } // while maxNum exceeds 3 characters, remove last digit from string.

    var intNum = parseInt(inputNum.value); if (intNum < minInputNum) { inputNum.value = minInputNum; } else if (intNum > 999) { inputNum.value = 999; } // If num is exceeds boundary, fill it with boundary maximums.
    var intNum = parseInt(inputNum.value); if (intNum > 9 && intNum < 1000 && minInputNum == 10) { document.getElementById("maxNumSli").value = intNum; } // Update slider if is present and num is conforms to boundary.
}
...
if (document.getElementById("maxNumSli") != null) { document.getElementById("maxNumSli").addEventListener("input", function() {
...
if (document.getElementById("timePassed") != null && document.getElementById("hasWon") == null) {
    ...
    setTimeout(function(){ updateTime(); }, 1000); // Run function that count time upwards every second.
}
...
function updateTime() {
    ...
    setTimeout(function(){ updateTime(); }, 1000); // Run function that updates time every second.
}